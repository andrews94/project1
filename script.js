var wordBank = prompt('Enter a word to play').toUpperCase().split("");
var wordInPlay = [];
var error = 0
var success = 0
for (var i = 0; i < wordBank.length; i++){
	wordInPlay.push("_")
	$(".wordDisplay").append(`<span class=${i}>_ </span>`)
}

function checkChar (){
	let userGuess = $(".userinput").val().toUpperCase();
	let guessBoolean = wordBank.includes(userGuess)
	console.log(guessBoolean);
	if (guessBoolean) {
		replaceChar(userGuess);
	} else {
		$(`#incorrectLetter`).append(`<span>${userGuess} </span>`)
		error ++;
		hangman.src = "imgs/hang" + error + ".png";
		console.log(error);
	}

	if(success == wordBank.length){
		sound2 ();
...
function replaceChar (userGuess){
...
// var bank = new Array;
// wordArray = prompt('Enter a word to play').toUpperCase().split("");
